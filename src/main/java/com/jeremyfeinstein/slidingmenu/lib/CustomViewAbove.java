...
public class CustomViewAbove extends ViewGroup {
    ...
    public void setContent(View v) {
        if (mContent != null) { this.removeView(mContent); }
        mContent = v;
        addView(mContent);
    }
    ...
    private boolean thisSlideAllowed(float dx) {
        ...
        if (DEBUG) { Log.v(TAG, "this slide allowed " + allowed + " dx: " + dx); }
        return allowed;
    }
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_DOWN:
                ...
                if (thisTouchAllowed(ev)) {
                    ...
                    if (isMenuOpen() && mViewBehind
                            .menuTouchInQuickReturn(mContent, mCurItem, ev.getX() + mScrollX)) {
                        mQuickReturn = true;
                    }
                } else {
        ...
        switch (action & MotionEventCompat.ACTION_MASK) {
            ...
            case MotionEvent.ACTION_UP:
                if (mIsBeingDragged) {
                    ...
                    endDrag();
                } else if (mQuickReturn && mViewBehind
                        .menuTouchInQuickReturn(mContent, mCurItem, ev.getX() + mScrollX)) {
                    // close the menu
                    setCurrentItem(1);
                    endDrag();
    ...
    private void determineDrag(MotionEvent ev) {
        ...
        if (xDiff > (isMenuOpen() ? mTouchSlop / 2
                : mTouchSlop) && xDiff > yDiff && thisSlideAllowed(dx)) {
            startDrag();
            mLastMotionX = x;
            mLastMotionY = y;
            setScrollingCacheEnabled(true);
