            ...
            public void onClick(View view) {
                switch (state) {
                    case Constants.UPDATE_DOWNLOADED:
                        try {
                            Helper.triggerUpdate(Application.sApplicationContext,
                                    mUpdateInfo.getUpdateName() + ".zip");
                        } catch (Exception exc) {
                    ...
                    case Constants.UPDATE_DOWNLOADING:
                        final DownloadManager downloadManager =
                                (DownloadManager) Application.sApplicationContext
                                        .getSystemService(Context.DOWNLOAD_SERVICE);
                        downloadManager.remove(Long.parseLong(downloadItem.getDownloadId()));

                        final DatabaseHandler db =
                                DatabaseHandler.getInstance(Application.sApplicationContext);
                        db.deleteItem(downloadItem, DatabaseHandler.TABLE_DOWNLOADS);
                    ...
                    default:
                        final Intent i =
                                new Intent(Application.sApplicationContext, DownloadReceiver.class);
                        i.setAction(DownloadReceiver.ACTION_START_DOWNLOAD);
                        i.putExtra(DownloadReceiver.EXTRA_UPDATE_INFO, (Parcelable) mUpdateInfo);
                        Application.sApplicationContext.sendBroadcast(i);
                        break;
                }
            }
        });
